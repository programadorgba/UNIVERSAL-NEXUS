...
const StarwarsCharacterGrid = () => {
  ...
  const loadCharacters = useCallback(async (pageNum, isInitial = false) => {
    ...
    try {
      ...
      switch(activeCategory) {
        case 'planets': data = await starWarsAPI.getPlanets(pageNum); break;
        case 'starships': data = await starWarsAPI.getStarships(pageNum); break;
        case 'vehicles': data = await starWarsAPI.getVehicles(pageNum); break;
        case 'species': data = await starWarsAPI.getSpecies(pageNum); break;
        case 'films': data = await starWarsAPI.getFilms(pageNum); break;
        default: data = await starWarsAPI.getPeople(pageNum);
      }
  ...
  return (
    <div className="character-grid-page" style={{ background: 'radial-gradient(circle at top, #000000 0%, #1a1a1a 100%)' }}>
      <aside className="sidebar" style={{ borderRightColor: 'rgba(251, 191, 36, 0.3)' }}>
        <div className="sidebar-logo" onClick={() => navigate("/")} style={{ borderColor: 'rgba(251, 191, 36, 0.4)', background: 'radial-gradient(circle at top, rgba(251, 191, 36, 0.2), transparent 70%)' }}>
          ...
          {categories.map((category) => (
            <button
              ...
              style={activeCategory === category.id ? { 
                ...
                background: 'radial-gradient(circle at left, rgba(251, 191, 36, 0.4), rgba(15, 23, 42, 0.95))'
              } : {}}
      ...
      <main className="main-content">
        <header className="grid-header">
          ...
          <form className="search-form" onSubmit={handleSearch}>
            ...
            />
            <button type="submit" className="search-btn" style={{ background: 'linear-gradient(135deg, #fbbf24 0%, #d97706 100%)', color: '#000' }}>
              <Search size={20} />
              Buscar
            </button>
          </form>
        ...
        {error && <div className="error-message" style={{ borderLeft: '4px solid #fbbf24' }}>{error}</div>}
        ...
        <div className="characters-grid">
          {filteredCharacters.map((char) => (
            ...
            >
              ...
              <div className="card-image-container" style={{ background: 'radial-gradient(circle at top, #fbbf24, #000)' }}>
                <img
                  src={char.image}
                  alt={char.name}
                ...
                <div className="card-badges">
                  ...
                  >
                    {activeCategory === 'people' 
                      ? (char.gender === 'n/a' ? 'Droid' : char.gender)
        ...
        {!loading && hasMore && filteredCharacters.length > 0 && activeCategory === 'people' && !searchTerm && (
          <div className="load-more-container">
            <button 
              className="load-more-btn" 
              onClick={() => loadCharacters(page)}
              style={{ background: 'linear-gradient(135deg, #fbbf24 0%, #d97706 100%)', color: '#000', borderColor: '#fbbf24' }}
            >
