...
const test = async () => {
    try {
        ...
        const searchResponse = await axios.get('https://superhero-backend-yf8q.onrender.com/api/superhero/search/Batman');
        if (searchResponse.data.success && searchResponse.data.data.length > 0) {
            ...
            const detailsResponse = await axios.get(`https://superhero-backend-yf8q.onrender.com/api/superhero/character/${batman.id}`);
        ...
        const originalResponse = await axios.get('https://superhero-backend-yf8q.onrender.com/api/superhero/character/620');
